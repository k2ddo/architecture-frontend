...
import {
  getInfoMessage,
  searchChatMessages,
} from '@/api/sc/search/searchChatMessages'
import {
  newMessageAgent,
  createLinkText,
} from '@/api/sc/agents/newMessageAgent'
...
export const useChat = (user: ScAddr | null) => {
  ...
  const initChat = useCallback(async (users: ScAddr[]) => {
    ...
    const { messages, shouldEnd } = await searchChatMessages(
      chatNode,
      minNumberMessages()
    )
    if (!messages) return
  ...
  const onFetching = useCallback(async () => {
    ...
    const { messages: newMessages, shouldEnd: localShouldEnd } =
      await searchChatMessages(chatNode, minNumberMessages(), lastMessageScAddr)
  ...
  const sendMessage = useCallback(
    async (user: ScAddr, text: string, setMessage: boolean = true) => {
