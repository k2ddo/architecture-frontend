...
export const createLinkText = async (messageText: string) => {
  ...
  constructionLink.createLink(
    ScType.LinkConst,
    new ScLinkContent(messageText, ScLinkContentType.String)
  )
...
const describeAgent = async (
  ...
  template.triple(keynodes[question], ScType.EdgeAccessVarPosPerm, [
    ScType.NodeVar,
    actionNodeAlias,
  ])
  template.triple(
    keynodes[actionReplyToMessage],
...
const findNewMessageNode = async (circuitAddr: ScAddr) => {
  ...
  template.triple(circuitAddr, ScType.EdgeAccessVarPosPerm, [
    ScType.NodeVar,
    message,
  ])
