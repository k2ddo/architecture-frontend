...
const findLastMessageNode = async (
  chatNode: ScAddr,
...
const findCurrentEdge = async (chatNode: ScAddr, messageNode: ScAddr) => {
  ...
  template.triple(
    chatNode,
    [ScType.EdgeAccessVarPosPerm, currentEdgeAlias],
    messageNode
  )
...
const findPreviousMessageNode = async (
  chatNode: ScAddr,
  currentEdge: ScAddr
) => {
  ...
  templatePreviousMessageClass.triple(chatNode, currentEdge, [
    ScType.NodeVar,
    previousMessageClassAlias,
  ])

  const resultNextMessageClassEdge = await client.templateSearch(
    templatePreviousMessageClass
  )
...
const findPreviousEdge = async (
  chatNode: ScAddr,
  keynodes: Record<string, ScAddr>,
  currrentMessageNode: ScAddr
) => {
  ...
  templateNextEdge.triple(
    chatNode,
    [ScType.EdgeAccessVarPosPerm, nextMessageEdgeAlias],
    ScType.NodeVar
  )
  templateNextEdge.triple(
    chatNode,
    [ScType.EdgeAccessVarPosPerm, currentMessageEdgeAlias],
    currrentMessageNode
  )
  ...
  const resultNextMessageClassEdge =
    await client.templateSearch(templateNextEdge)
  if (resultNextMessageClassEdge.length) {
...
export const getInfoMessage = async (
  ...
  template.triple(messageNodeAlias, ScType.EdgeAccessVarPosPerm, [
    ScType.LinkVar,
    textNodeAlias,
  ])
  template.tripleWithRelation(
...
const getFirstMessages = async (
  ...
  let count = 0
  let currentMessagesNode =
    lastMessageScAddr || (await findLastMessageNode(chatNode, keynodes))
  ...
  while (count < amount) {
    ...
    currentEdge = await findPreviousEdge(
      chatNode,
      keynodes,
      currentMessagesNode
    )
  ...
  const shouldEnd = messages[0]
    ? await checkRrel1(keynodes, chatNode, messages[0].addr)
    : true
